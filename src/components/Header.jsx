    ...
    return (
        <>
            <header className='p-4 flex fixed top-0 left-0 right-0 z-20 bg-white/50 backdrop-blur'>
                <nav className='flex justify-between items-center w-full text-primary'>
                    <div className='font-bold text-base text-primary tracking-wide font-teko'>
                        <Link to="/" onClick={closeMenu}>kunalkcube.pro</Link>
                    </div>
                    <div className='flex justify-center items-center text-base font-semibold cursor-pointer' onClick={toggleModal}>
                        <RiStackLine
                            size={16}
                            color='black'
                        />
